...
- Light mode, the error colour, the border radius, and the dark text_high, action and btn_text of the Rauthy client themes — The estate colour tokens hold a dark foundation only and name no light palette, no error colour, no radius, no emphasised text tier, no accent for actions and no text-on-accent; each is a gap for a design-system card, and DIRECTORY-002 leaves each at Rauthy's own default (ADR-021).
...
| `.land/gates.sh` | the land gate; gains the identity leg that runs the container-backed identity tests and refuses container_runtime_missing | DIRECTORY-002 |
| `CLAUDE.md` | the project's standing instructions; its Gates before any commit list gains the identity leg as one line (CN9 wall revision) | DIRECTORY-002 |
...
| `crates/lys/src/identity/config_tests.rs` | unit tests of config.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/credentials_tests.rs` | unit tests of credentials.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/private_files_tests.rs` | unit tests of private_files.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/prepare_tests.rs` | unit tests of prepare.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/configure_tests.rs` | unit tests of configure.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/rauthy_tests.rs` | unit tests of rauthy.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/themes_tests.rs` | unit tests of themes.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/health_tests.rs` | unit tests of health.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
| `crates/lys/src/identity/error_tests.rs` | unit tests of error.rs, beside it (CN9 wall revision) | DIRECTORY-002 |
...
- **CN9** — A row that needs a file outside its wall stops and names it, and the reviewer approves a brief revision before that file is edited; a directory wall for a wholly new module allows only its named responsibility and needs an exact file manifest reviewed before its row starts (docs/design/identity/briefs/IDENTITY-001.json:28). Wall revisions approved for DIRECTORY-002: each module of the crates/lys/src/identity/ manifest may have a sibling <module>_tests.rs holding its unit tests, as crates/lys/src/cli_tests.rs does, and those are the only files added there beyond the manifest; and CLAUDE.md gains one line, the identity leg, in its Gates before any commit list and changes nowhere else.
