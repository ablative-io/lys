...
Carry IDENTITY-001's open rows (02, 04, 03, 05) into design-system briefs in this cluster, DIRECTORY-002 to DIRECTORY-005, revised for the grant ruling (ADR-003), the PostgreSQL ruling (ADR-005) and the working lifecycle states (ADR-011, proposed), with the fork (ADR-009) and the product accents (ADR-010) in the project ledger and every decision still open for Tom marked open. The IDENTITY-001 files stay as they are, as the record of revision 5. DIRECTORY-009 moves an identity between the four lifecycle states by signed record and gates every check on the folded state: every transition caused only by the actor the table names and recorded as one signed audit record of IDENTITY-001's shape, so the state is folded from the log and never set directly (ADR-027); the access check as the conjunction with the state leg first, so a suspended or retired identity fails by state alone; a suspension taking effect at the next check, where a session refresh is a check and nothing calls Rauthy to revoke (ADR-028); retired identities readable as live ones; and the typed read the lifecycle screen consumes. Its files live in crates/lys-identity/src/lifecycle/ and beside the directory service, and it edits DIRECTORY-003's and DIRECTORY-006's files only by named added lines.
