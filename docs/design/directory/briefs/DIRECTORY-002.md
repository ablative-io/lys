...
- Comparing each hex value in deploy/identity/theme-map.md's bg, bg_high, text and accent rows with the value of the token its row names in docs/design-system-v2/palette/estate-colour-tokens.json at ablative-docs 385916e, token by token by name, counts zero mismatches.
...
- From the repository root: cargo fmt --check; cargo clippy --all-targets --all-features -- -D warnings; cargo clippy --all-targets -- -D warnings; cargo test --workspace --all-features; cargo doc --no-deps --all-features; cargo doc --no-deps; and the identity leg, cargo test -p lys --all-features --test 'identity_*', run through .land/gates.sh on a host with a container runtime, all clean (CLAUDE.md, gates before any commit).
