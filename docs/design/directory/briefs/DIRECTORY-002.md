...
- At the brief's src commit, every decision in docs/design/decisions.json at commit 1756688 and every row in docs/design/roadmap.json at commit 1756688 is present by id and byte-identical when serialised with sorted keys, ADR-021 appears exactly once, and python3 scripts/design/validate.py docs/design/roadmap.json exits 0.
