...
Note on the sealed-envelope row: the salt, `info` layout, OKM length, key/nonce split and AAD stated above are a **completion of this table, not a change to a frozen value** — every one of them is the as-built behaviour, unchanged. They are recorded here because a necessity test found that **none of them was pinned by anything executable**: reversing the two public keys in the `info`, and changing the domain tag itself, each left the entire suite green. The derivation was previously stated only in `lys-core/DESIGN.md` and `CHECKLIST.md` C45 — i.e. the table designated authoritative said *less* than its subordinate documents, and the salt, OKM length, split and AAD were written down nowhere at all. **Prose does not run in CI**, so the statement above is now pinned by `lys-core/tests/seal_derivation.rs`, which re-derives the key material from this specification — hardcoding the literal rather than importing `SEAL_INFO`, so it cannot move with the code it checks — and which is proven to fail against each drift rather than merely observed to pass.
...
- **`tree_size` is bound by the checkpoint, and it is NOT redundant with the proof** — the heading this bullet used to carry ("redundancy is checked, not trusted") described the rule as belt-and-braces when it is load-bearing. `tree_size` MUST equal the embedded checkpoint's line-2 value, and the root recomputed from the proof MUST equal the checkpoint's line-3 hash; `lys log verify` rejects mismatches. **Why the size check cannot be dropped:** under RFC 6962 the inclusion path's length *and sidedness sequence* are shared across ranges of tree size — for leaf 3, sizes 5, 6, 7 and 8 all produce the identical shape, changing only at 9. So an artifact built for a five-leaf tree and relabelled `tree_size: 6` still walks to the five-leaf root and still matches the five-leaf checkpoint's hash. A verifier that recomputes the root but never compares the size reports *"leaf 3 of 6"* about a five-leaf log, with every hash it checked agreeing. The signed checkpoint is the only thing pinning the size. This is the same residual named for receipts in `WIRE-DRAFTS.md` (§1.3.1, "at some `(index, size)` pairs two sizes share a walk") — stated there and mislabelled here, which is why it is corrected rather than restated. A proof without its checkpoint is unverifiable, which is why the checkpoint rides inside the artifact.
...
### 4.2 RATIFIED (D4) — attestations: COSE_Sign1, the only attestation artifact
...
| D4 | Attestation artifact = tagged COSE_Sign1 (`lys/attestation/v2`), alg = EdDSA(-8) after the deployed-practice check, protected content-type + kid domain binding, deterministic CBOR, canonical-strict verifier; the v1 JSON artifact deleted unshipped (§4.2) | **RATIFIED — 2026-07-10, Tom Whiting** (Meridian exchange). Direction approved 2026-07-10 ("if that seems like a pretty strong recommendation from you… let's proceed", including the ruling that nothing migrates); ratified on the maintainers' recommendation in lieu of a byte-exact read ("if that's absolutely definitely the best way forward then go for it. You absolutely have the blessing there"). Recorded honestly: the operator ratified the recommendation and its plain-language walkthrough, not the byte layout line-by-line; the §4.2 rules stand on the conformance evidence in `lys-core/tests/cose_conformance.rs` (vendored `veraison/go-cose` v1.3.0). Substance in `lys-core/src/attestation/` module docs |
...
| D6 | Conformance testing for D1/D2 includes vectors verified against the Go `sumdb/note` reference, not only Rust implementations (§2.2) | **RATIFIED — 2026-07-10, Tom Whiting** (same exchange as D1; his amendment: pursue the strongest independent-authorship evidence available — satisfied by a second independent implementation: `lys-core/tests/signed_note_crosscheck.rs` round-trips against Cloudflare's `signed_note` crate, byte-identical both ways). Evidence: `lys-core/tests/go_conformance.rs` round-trips sign/verify against the vendored `golang.org/x/mod/sumdb/note` v0.22.0, byte-identical both ways |
