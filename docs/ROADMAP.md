...
- `lys attest` / `lys verify` — sign and verify `lys/attestation/v2` COSE_Sign1 artifacts over a payload file. `verify --cert --issuer-public-key` additionally requires that the certificate verifies and that the attestation's signer *is* the key it certifies, so one exit code answers "did this named subject, holding these capabilities, make this statement?" — without it, a caller is left comparing two hex strings by eye, and both halves pass while saying nothing about each other.
...
- `lys inspect attestation` / `lys inspect cert` — read-only viewers for a recipient who was handed an artifact: print what it *says* without verifying any of it. Every output opens with an `UNVERIFIED` banner naming the command that does verify, and capability claims carry a second marker because anyone can put anything there. Local files only — a decode-success signal is a parsing oracle if exposed over a network.

**Proof — demonstrated 2026-07-11, and re-established as a committed gate 2026-08-08:** a log built in one directory was verified in a bare directory holding only the artifact, the leaf file, and the verifier-key string; the leaf hash reproduces with `shasum`, and the root recomputes from the artifact with a standalone stdlib-only script — `scripts/verify_inclusion.py`, 175 lines, of which the RFC 6962 recomputation itself is 15.
