...
**What is and is not published, as of 2026-08-08:** `lys-core` 0.2.0, `lys` 0.2.0 and `lys-log-store` 0.2.0 are on crates.io. `lys-anchor` and `lys-anchor-cli` are `publish = false`. **`lys/delegation/v1` is NOT published** — renamed today from `lys/anchor-` + `delegation/v1`, along with its content type, because a format that also serves seats should not say *anchor* in signature-covered bytes. Being unpublished is the only reason **three** payload changes were free in one day: `sequence` (the replay fix), the typed subject replacing `origin`, and the role renumbering to `{2, 3}` that keeps `subject_kind == role` from being true of every valid artifact. Publishing `lys-core` 0.3.0 freezes all of it — so that release waits until the fold that enforces the ordering rule exists, because until then the format's central security property is carried by prose.

**Note what that day cost, because it is the argument for specifying before implementing rather than a war story:** every one of those three changes was found by a *second party* — an adversarial review, a real consumer arriving, and an independent encoder reasoning about the enum space. None was found by a test, and the third one **could not have been**: under roles numbered from `1`, a swap of the `subject_kind` and `role` fields produced byte-identical output for every valid artifact, so no test, vector, or COSE library could distinguish the wired-correctly implementation from the wired-backwards one. A test suite can only see distinctions the encoding makes.
